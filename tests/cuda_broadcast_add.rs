//! CUDA mid-axis broadcast add parity (EEG channel embedding pattern).

use rlx::ir::op::BinaryOp;

#[test]
#[cfg(feature = "rlx-cuda")]
fn cuda_channel_broadcast_add_matches_cpu() {
    ...
    let mut cuda = rlx::Session::new(rlx::Device::Cuda);
    let mut gc = cuda.compile(g);
    let got = gc
        .run(&[("emb", &emb_v), ("ch", &ch_v)])
        .into_iter()
        .next()
    ...
    assert!(
        err < 1e-5,
        "channel broadcast add max_abs={err:.3e} (cpu[100]={} cuda[100]={})",
        want[100],
        got[100]
    );
}
